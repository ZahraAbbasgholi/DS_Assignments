...
System.Console.WriteLine((int)char.ToUpper('d') + 32);

const long BigPrimeNumber = 1000000007;
const long ChosenX = 263;
long bucket = 5;
long calculate_number(string str)
{
    str = str.ToLower();
    double value = 0;
    for (int i = 0; i < str.Length; i++)
    {
        value += ((int)char.ToUpper(str[i]) + 32) * (Math.Pow(ChosenX, i));
    }
    return (((long)value % BigPrimeNumber) % bucket);
}
calculate_number("HellO");
// using System.Text;
...
//     {
// Q1PhoneBook ml = new Q1PhoneBook("lsj");
// var path = @"E:\DS\Assignments\A10\A10.Tests\TestData\A10.TD1\In_1.txt";
